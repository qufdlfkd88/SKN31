...
select emp_id as 직원ID,   -- emp_id 직원ID //as 생략 가능
       emp_name as 직원이름, -- 별칭에 공백이 들어가면 ""로 묶어준다
       hire_date as 입사일, 
       salary as 급여, 
       dept_name as 소속부서
from   emp;
...
from emp
-- where comm_pct between 0.2 and 0.3; -- 0.2 와 0.3을 포함한 안의 값
where comm_pct not between 0.2 and 0.3; -- 0.2보다는 같거나 작고 0.3 보다는 같거나 큰것
-- where comm_pct >= 0.2 and comm_pct <= 0.3; // 대체 가능
...
-- where job not in ('IT_PROG', 'ST_MAN'); // IT_PROG, ST_MAN값이 없는 나머지들
-- where job = 'IT_PROG' or job = 'ST_MAN'; // 대체 가능

-- EMP 테이블에서 직원 이름(emp_name)이 S로 시작하는 직원의  ID(emp_id), 이름(emp_name)을 조회.
select emp_id, emp_name
from emp
where emp_name like 'S%';
-- where emp_name like '%en'; // en으로 끝나는사람
-- where emp_name not like 'S%'; // S% 가 아닌 나머지 사람 
-- where emp_name ='S%'; //S% 인 사람 
...
select emp_id, emp_name
from emp
where emp_name like '%$%%' escape '$'; -- escape '문자' 문자%, 와 문자_ %와 _를 그대로 비교
-- % $% % 앞뒤 %는 문자들 중간 $%는 %를 포함한
...
select *
from emp
where comm_pct is not null; -- comm_pct의 비어 있는 값이 없음
...
-- where hire_date between '2004-01-01' and '2004-12-31'; -- 위와 동일
...
order by 3 asc, 4 desc; -- select 절에 job이 세번째 있기때문에 3, salary가 네번째 있기 때문에 4
