...
select  lpad(emp_name, 15, ' ') "emp_name",   -- 일괄적으로 글자수를 맞추기 위해 사용한다.
...
-- 날짜 타입에서 년 월 일 조회
select year(now()) "년도",
       month(curdate()) "월", -- curdate() 대신 now() 사용해도 무방
       day(curdate()) "일"
;
select date(now());
select time(now());
...
select subdate(curdate(), interval 10 month), -- 10개월을 뺀 년월일
       curdate(),
       adddate(curdate(), interval 10 month)  -- 10개월을 더한 년월일
;
select adddate(now(), interval 3 week) "3주 후",
	   adddate(now(), interval 10 week) "10주 후",
	   adddate(now(), interval -10 hour) "10시간 전",
	   adddate(now(), interval 10 hour) "10시간 후";
       
-- 날짜/시간의 차이 계산
select datediff(curdate(), '2024-10-10'); -- 오늘 - 2024-10-10 
...
select emp_name,
       date_format(hire_date, '%Y년 %m월 %d일') "년월일시분초"
from emp
where emp_id = 200;

select date_format(now(), '%Y년 %m월 %d일 %H시 %i분 %s초') "년월일시분초";
...
select emp_id, emp_name, job, dept_name, ifnull(dept_name, '----배치 전') "dept_name"
from   emp;
...
select salary,
	   if(salary >= 10000, 'A등급', 'B등급') "salary 등급"
from   emp;
...
select salary,
       case when salary >= 20000 then '1등급' 
            ...
            else '2등급' -- where salary 10000 and 19999
	   end "급여 등급"
from emp;
