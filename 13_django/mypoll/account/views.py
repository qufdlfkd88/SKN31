...
def create(request):
    ...
    if request.method == "GET":
        ...
        if form.is_valid(): # ValidationError가 발생하지 않았으면(검증 통과)
            ...
            user = form.save() # ModelForm을 이용해 Insert/Update처리. 처리한 Model 객체를 반환
...
def user_login(request):
    ...
    if request.method == "GET":
        ...
        if user is not None: # 일치
            ...
            # 로그인해야 요청 할 수 있는 view를 로그인 하지 않고 요청한 경우, 로그인 페이지로 이동하면서
...
#       POST-회원정보조회 페이지로 이동(account:detail view:redirect)
###################################################################
@login_required
def update(request):
    if request.method == "GET":
        # 수정폼 생성: 수정할 Model 객체를 전달 -> 화면에 기존 데이터가 입력폼에 나오도록한다.
...
#           GET-변경 폼, POST-변경처리
# 응답: GET-account/password_change.html, POST-redirect: account:detaul 뷰
##########################################################################
@login_required
